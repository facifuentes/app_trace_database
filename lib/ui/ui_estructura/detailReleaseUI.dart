...
import 'package:trace_database/ui/ui_estructura/ComparationUI.dart';
...
class DetailReleaseUI extends StatelessWidget {
  ///The detailhc is sent from hc_Patien_ui
  final String schema;
  final String idRelease;
  DetailReleaseUI({this.schema, this.idRelease});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        body: DetailReleaseListPage(
      schema: schema,
      idRelease: idRelease,
    ));
  }
...
class DetailReleaseListPage extends StatefulWidget {
  final String schema;
  final String idRelease;
  DetailReleaseListPage({this.schema, this.idRelease});

  @override
  _DetailReleasetate createState() =>
      _DetailReleasetate(schema: schema, idRelease: idRelease);
}

class _DetailReleasetate extends State<DetailReleaseListPage> {
  final String schema;
  final String idRelease;
  _DetailReleasetate({this.schema, this.idRelease});
  ...
  initState() {
    ...
    consultBloc.getRelase(schema, idRelease);
          ...
          if (consultBloc.listDropdown.length == 0) {
            _warnCreateBaseLine().then(
              (response) {
                if (response) {
                  consultBloc
                      .updateBaseLine(schema, idRelease, idRelease)
              ...
              (response) {
                if (response) {
                  Navigator.of(context).push(MaterialPageRoute(
                      builder: (BuildContext context) => ComparationUI(
                            schema: schema,
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                            ...
                            children: <Widget>[
                              ItemListTile(
                                title: "Estructura: ",
                                description: snapData[index].estructura,
                              ),
                              ItemListTile(
                                title: "Huella: ",
